...
| **v1.26.0** | **Entity Lifecycle + Reporting Calendar Foundation — design pin (docs-only).** Designs a generic, country-neutral substrate adding `UniverseEventRecord` (closed-set `UNIVERSE_EVENT_TYPE_LABELS`: entity_listed / delisted / merged / renamed / split / status_changed / unknown) + `ReportingCalendarProfile` (closed-set `MONTH_LABELS` 1–12 + unknown; `DISCLOSURE_CLUSTER_LABELS` concentrated / moderate / dispersed / unknown; `REPORTING_INTENSITY_LABELS` low / medium / high / unknown) + read-only `UniverseCalendarReadout` (active / inactive entity ids, lifecycle event counts, reporting-due entity ids, fiscal-year-end month distribution). v1.26.0 = docs-only design; v1.26.1 = `UniverseEventRecord` storage; v1.26.2 = `ReportingCalendarProfile` storage; v1.26.3 = read-only universe-calendar readout; v1.26.4 = optional descriptive-only export section + optional minimal "Universe / calendar" panel inside the existing Universe sheet (no new tab); v1.26.last = freeze. **Empty-by-default kernel fields preserve every existing fixed fixture byte-identically** — a kernel without UniverseEventRecord + ReportingCalendarProfile records continues to behave exactly as it did at v1.25.last. **Synthetic only. No real data. No Japan calibration. No EDINET / TDnet / J-Quants / JPX / TOPIX / Nikkei / GICS / MSCI / S&P / FactSet / Bloomberg / Refinitiv dependency. No event-to-price mapping. No earnings-surprise / event-study / calendar-arbitrage inference. No portfolio / universe weight / constituent weight / rebalance event. No actor decision. No source-of-truth book mutation. No digest movement.** Independent of v1.27; advances on its own cadence. | **Design scoped — current.** See [`docs/v1_26_entity_lifecycle_reporting_calendar_foundation.md`](japan-financial-world/docs/v1_26_entity_lifecycle_reporting_calendar_foundation.md) and [`docs/world_model.md`](japan-financial-world/docs/world_model.md) §135. |
