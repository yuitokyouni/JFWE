...
def test_exchange_space_can_read_latest_price():
    ...
    kernel.prices.set_price("asset:toyota_eq", 2_550.0, "2026-01-02", "exchange")
...
def test_exchange_space_can_read_price_history():
    ...
    kernel.prices.set_price("asset:toyota_eq", 2_500.0, "2026-01-01", "exchange")
    kernel.prices.set_price("asset:toyota_eq", 2_550.0, "2026-01-02", "exchange")
    kernel.prices.set_price("asset:toyota_eq", 2_510.0, "2026-01-03", "exchange")
...
def test_exchange_space_can_read_visible_signals():
    ...
    kernel.signals.add_signal(
        InformationSignal(
            ...
            subject_id="asset:toyota_eq",
            source_id="market:tse",
...
def test_exchange_space_does_not_mutate_world_books():
    ...
    exchange = ExchangeSpace()
    kernel.register_space(exchange)
    exchange.add_market_state(MarketState(market_id="market:tse"))
    exchange.add_listing(
        ListingState(market_id="market:tse", asset_id="asset:toyota_eq")
    )

    # Read every projection through the space.
    exchange.get_latest_price("asset:toyota_eq")
    exchange.get_price_history("asset:toyota_eq")
    exchange.get_visible_signals("market:tse")
    exchange.list_assets_on_market("market:tse")
    exchange.snapshot()
