...
| v1.16.2       | Living-world classifier rewire — replaces the v1.15.5 four-cycle `(period_idx + investor_idx + firm_idx) % 4` rotation in `world/reference_living_world.py` with a per-pair call into `classify_market_intent_direction(...)`; resolves classifier inputs from cited records (investor intent, valuation, firm financial state, market environment, actor attention); `InvestorMarketIntentRecord.metadata` now carries a classifier-audit block (`classifier_version` / `classifier_rule_id` / `classifier_status` / `classifier_confidence` / `classifier_unresolved_or_missing_count` / `classifier_evidence_summary`); `living_world_digest` moves to `0b75e95ad8f157df5e938c1318817c07f00798179c3d11b8629452d30d9398fa` by design (label / metadata bytes only — record count and per-run window unchanged); +16 tests | Shipped                 |
| v1.16.3       | Securities-market pressure → next-period attention feedback — closes the v1.12 endogenous-attention loop with the v1.15 securities-market-pressure / corporate-financing-path loop; `world/attention_feedback.py` adds five closed-set focus labels (`risk` / `financing` / `dilution` / `market_interest` / `information_gap`), two new trigger labels (`market_pressure_observed` / `financing_path_observed`), two new source-id slots on `ActorAttentionStateRecord` (`source_indicative_market_pressure_ids` / `source_corporate_financing_path_ids`), two new `build_attention_feedback` kwargs, and two deterministic closed-set rule helpers (`_classify_market_pressure_focus` / `_classify_financing_path_focus`); orchestrator threads the previous period's pressure / path ids into the next period's attention build; v1.12.9 budget / decay / saturation discipline preserved bit-for-bit; **no new records** — slot additions only — per-period record count and per-run window unchanged; `living_world_digest` moves to `f93bdf3f4203c20d4a58e956160b0bb1004dcdecf0648a92cc961401b705897c` by design; +34 tests | Shipped                 |
| **v1.16.last** | **Endogenous Market Intent Feedback freeze** (docs-only) — single-page reader-facing summary in [`v1_16_endogenous_market_intent_feedback_summary.md`](v1_16_endogenous_market_intent_feedback_summary.md); §118 in `docs/world_model.md`; v1.16.last release-readiness snapshot in `RELEASE_CHECKLIST.md`; v1.16.last freeze-pin section in `docs/performance_boundary.md`; v1.16.last test-inventory header note; v1.16.last addendum in `examples/reference_world/README.md`; v1.16.last cross-link in `docs/fwe_reference_demo_design.md`. Test count, per-period record count, per-run window, and `living_world_digest` all unchanged from v1.16.3 by design. First public-FWE closed endogenous-market-intent feedback loop. | **Shipped (docs-only)** |
