...
| Phase                                                    | Loop shape                                | v1.11.1 default        |
| -------------------------------------------------------- | ----------------------------------------- | ---------------------- |
...
| Reporting / replay / manifest                            | `O(R)` over emitted ledger records        | ~262 records           |

Per-period record-count breakdown (default fixture, v1.11.1):
...
the per-run total is `61 × 4 = 244`, plus an infrastructure
allowance for one-off setup records (14 v1.9.x infra + 4 v1.10.5
stewardship themes + headroom; v1.11.0 / v1.11.1 add no new
setup records).

- per-period record formula equals
  `2F + F + N + M + K + 2(I+B) + IF + BF + IF + IF + F + 2(I+B) = 61`
  for the default fixture (with `N = 3` industries,
  `M = 5` markets, and `K = 1` capital-market readout per
  period),
- per-run total record count for a default 4-period sweep sits
  in `[244, 276]` — i.e. exactly `formula × periods` at the
