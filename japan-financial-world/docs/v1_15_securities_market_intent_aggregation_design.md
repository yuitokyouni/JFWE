...
| v1.15.5     | Living-world integration — per-period `I × F` market intents + `F` aggregated-interest + `F` indicative-pressure records; setup adds 1 venue + `F` listed securities; `living_world_digest` moves to `041686b0…03403a5`; per-run window `[432, 480]`; +14 integration tests | Shipped                 |
| **v1.15.6** | **v1.14 feedback wiring** — `CapitalStructureReviewCandidate.source_indicative_market_pressure_ids` + `CorporateFinancingPathRecord.indicative_market_pressure_ids` citation slots; `build_corporate_financing_path` gains `indicative_market_pressure_ids` kwarg with constraint / coherence override rules; orchestrator reorders v1.15.5 to run before v1.14.5 so reviews and paths cite this-period pressure; `living_world_digest` moves to **`bd7abdb9a62fb93a1001d3f760b76b3ab4a361313c3af936c8b860f5ab58baf8`** by design (phase reorder + new citation slots); record count unchanged at 108 / 110 per period; +20 tests | **Shipped**             |
