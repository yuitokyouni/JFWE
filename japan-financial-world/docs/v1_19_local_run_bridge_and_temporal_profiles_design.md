...
| v1.19.1       | `world/run_export.py` — `RunExportBundle` immutable dataclass + JSON writer with `sort_keys=True` and `stable_for_replay` default; `RunProfileLabel` / `GeneratedAtPolicyLabel` closed-set frozensets; trip-wire test that the same `(profile, regime, scenario, seed)` produces a byte-identical bundle; default-fixture `living_world_digest` unchanged. | Planned                 |
...
v1.19.2 lands the CLI exporter. v1.19.3 lands the
`monthly_reference` profile *and* the
