...
| **v1.8.5 Corporate Reporting Routine** *(replaced)* | The original v1.8.5 plan was the corporate reporting routine. The realised v1.8.5 instead landed `AttentionProfile` / `ObservationMenu` / `SelectedObservationSet` storage (§47). The corporate reporting routine landed at **v1.8.7** instead, after the routine engine plumbing landed at v1.8.6. See the v1.8.7 row below. | (superseded) |
| **v1.8.7 Corporate Quarterly Reporting Routine** | First concrete routine: `corporate_quarterly_reporting`, on the diagonal `Corporate → Corporate` self-loop. Writes one synthetic `corporate_quarterly_report` `InformationSignal` per call through `RoutineEngine` + `SignalBook`. Synthetic only; no economic computation. **Shipped** (26 tests; 999 → 1025 total). | Yes (one routine) |
| **v1.8.8 Reference Variable Layer — Design** | Names the universe of observable world-context variables (macro / financial / material / energy / technology / real-estate / labor / logistics / expectations) so v1.8.12's investor + bank demo has *something to watch*. Anti-scenario discipline: variables are observable context, not shocks; absence is partial / degraded, not silent. v1.8.8 was *hardened* with the source / scope / exposure hooks and the **four-gate rule** (visibility → availability → selection → consumption) so variable movement does NOT auto-trigger routines — see the "Hardening" section in `v1_reference_variable_layer_design.md`. The hardening also formalises the responsibility chain `ReferenceVariable → VariableObservation → ExposureRecord → AttentionProfile → Routine` so each link requires explicit data, not implicit propagation. Design only. | No |
| **v1.8.9 `WorldVariableBook` / `IndicatorBook`** | Code: `ReferenceVariableSpec` + `VariableObservation` records, append-only revision history with `vintage_id` / `revision_of`, `list_released_as_of(...)` helper for look-ahead-bias-free reads. | Yes (kernel) |
| **v1.8.10 Exposure / Dependency Layer** | Code: per-actor declarations of which variables / groups the actor is *exposed to* (e.g., a refinery is exposed to `oil_price_reference`). Distinct from `AttentionProfile.watched_*` (what the actor *watches*); exposure is *what affects* the actor. v1.8.10 stores exposures only; transmission is later. | Yes (kernel) |
| **v1.8.11 `ObservationMenu` builder** | Code: helpers that build an `ObservationMenu` automatically by joining an `AttentionProfile` against `ReferenceVariableBook` (filtered for as-of look-ahead) + `SignalBook` + the other v0/v1 books. | Yes (kernel) |
| **v1.8.12 Attention Variable Hooks + Investor / Bank Attention Demo** | Code: `AttentionProfile` extended additively with `watched_variable_ids` / `watched_variable_groups` / `watched_exposure_types` / `watched_exposure_metrics`; `world/reference_attention.py` ships `run_investor_bank_attention_demo` which writes one menu and one `SelectedObservationSet` per actor. Selection only — no review routine, no economic mutation. **Shipped** (32 tests; 1225 → 1257 total). | Yes (kernel + demo) |
| **v1.8.13 Investor / Bank Review Routines** | Code: two narrow concrete consumer routines, `investor_review` (Investors → Investors self-loop) and `bank_review` (Banking → Banking self-loop), that read `SelectedObservationSet` records through the v1.8.6 `RoutineEngine` and emit one synthetic review-note `InformationSignal` each. The §49.4 / §49.5 review-routine sketches in this design are realised here, but **as audit artifacts only** — the notes carry count summaries, not economic interpretation. The full endogenous chain *corporate report → menus → heterogeneous selections → review run records → review notes* is now ledger-reconstructable. **Shipped** as `world/reference_reviews.py` (32 tests; 1257 → 1289 total). | Yes (two routines) |
| **v1.8.14 Endogenous Chain Harness** | Code: `world/reference_chain.py` ships `run_reference_endogenous_chain` and `EndogenousChainResult`. The harness orchestrates the v1.8.7 / v1.8.12 / v1.8.13 helpers in order — corporate quarterly reporting → heterogeneous attention → investor review → bank review — and returns a deterministic immutable summary that names every record id the chain wrote. **Orchestration only** — no new economic behavior, no new ledger record types, no scheduler auto-firing, no world-construction logic. The summary is convenience; the same chain is fully reconstructable from `kernel.ledger.records[before:after]`. The CLI `examples/reference_world/run_endogenous_chain.py` prints a compact human-readable trace. This is the smallest possible "everything fits together" demonstration that the v1.8.x stack composes correctly, and is the immediate prerequisite for v1.9's year-long sweep. **Shipped** (29 tests; 1289 → 1318 total). | Yes (harness) |
| **v1.9 Living Reference World Demo** | A new (or expanded) reference demo that runs for a full year on the routine + topology + attention + variable stack *without any external observation*. The ledger should contain meaningful records on every quarter-end, every month-end, every relationship review cycle. The replay-determinism gate must hold; the manifest must remain stable. | Yes (demo + tests) |
