...
| --------------------------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------- | ----------------------------------------------------------------------------------- |
| **v1.21 Stress Composition Layer**                       | thin orchestrator + read-only multiset readout over the existing v1.18 scenario-driver application chain. Reuses v1.18.1 `ScenarioDriverTemplate` ids verbatim; bundles them into a named, ordered `StressProgramTemplate`; applies them step-by-step via the existing v1.18.2 `apply_scenario_driver(...)` helper; emits **one program-level receipt** (`StressProgramApplicationRecord`) that *collects* the underlying `scenario_application_ids`; provides a read-only `StressFieldReadout` **multiset projection** per `(period, context_surface)`. **No interaction inference**, **no aggregate / composite / net / dominant fields**, **no `amplify` / `dampen` / `offset` labels** (deferred to v1.22+ as `manual_annotation`-only — never auto-inferred by a helper). Cardinality binding: ≤ 1 stress program per run, ≤ 3 stress steps per program, ≤ 60 v1.21 records added. **Append-only / read-only / no causality claim / no magnitudes / no price / no recommendation.** | **Shipped — v1.21.last frozen** (sequence: v1.21.0 → v1.21.0a correction → v1.21.1 storage → v1.21.2 thin orchestrator → v1.21.3 read-only multiset readout → v1.21.last docs-only freeze). +101 tests (4764 → 4865); v1.20.last digests preserved byte-identical. See [`v1_21_stress_composition_layer.md`](v1_21_stress_composition_layer.md) "v1.21.last freeze" section and `world_model.md` §130.8. |
| **v1.22 Static UI Stress Readout Reflection**            | reflects the v1.21.3 read-only multiset readout in the v1.20.5 static workbench's existing Universe tab. v1.22.1: new descriptive-only `stress_readout` payload section on `RunExportBundle` (empty-by-default; v1.21.last digests preserved byte-identical). v1.22.2: new "Active Stresses" strip above the existing sector heatmap, rendering one block per `stress_readout` entry — `as_of_date` / template id / `resolved / total` counter / "Partial application" badge when unresolved / context-surface / shift-direction / scenario-family multisets / citation lists / Raw canonical labels technical-details box. **No new tab. No bar height. No score. No `impact` / `outcome` / `risk score` / `prediction` / `recommendation` / `magnitude` wording. No backend; static JSON loaded via `<input type="file">` + `FileReader` only.** | **Shipped — v1.22.last frozen** (sequence: v1.22.0 design pin → v1.22.1 export section → v1.22.2 UI strip → v1.22.last docs-only freeze). +28 tests (4865 → 4893); v1.21.last digests preserved byte-identical. See [`v1_22_static_ui_stress_readout_reflection.md`](v1_22_static_ui_stress_readout_reflection.md) "v1.22.last freeze" section and `world_model.md` §131.9. |
| **v1.23 Substrate Hardening + Validation Foundation** | hardens the substrate that v1.21 / v1.22 now rely on (canonical-digest module, forbidden-token set composition, cross-layer metadata-stamp pin, runtime cardinality cap, test-inventory currency) and lays the validation foundation: 4 pinnable categories (determinism / boundary preservation / citation completeness / partial-application visibility) + 2 placeholders (inter-reviewer reproducibility / null-model comparison). Plus an attention-crowding read-only demo case study at v1.23.3. **No new dataclass / ledger event / label vocabulary. No no-stress behavior change. No digest movement.** | **Design scoped at v1.23.0** (docs-only — see [`v1_23_substrate_hardening_validation_foundation.md`](v1_23_substrate_hardening_validation_foundation.md) and `world_model.md` §132). v1.23.0 ships zero runtime / UI code; v1.23.1 / v1.23.2 / v1.23.3 are the implementation sub-milestones. |
| **v1.24 candidate — manual-annotation-only stress interaction layer** *(optional)* | a `manual_annotation`-only annotation layer over the v1.21.3 multiset readout: a human reviewer types in `interaction_label = amplify` / `dampen` / `offset` / `coexist` after reading the citations, with their own analyst id and timestamp on the annotation record, citing explicit evidence from a specific `(period, context_surface, active_stress_step_id_set)` cell. Closed sets `source_kind = {"human"}` and `reasoning_mode = {"human_authored"}`. **MUST NEVER be inferred by a helper, a classifier, a closed-set rule table, an LLM, or any other automated layer.** MUST NOT replace the multiset readout. | Optional candidate. Not currently planned. Would require a fresh design pin. |
| **v1.25 candidate — Institutional Investor Mandate / Benchmark Pressure** | benchmark / peer / mandate constraints on the v1.15.5 / v1.16.2 investor-intent layer; bounded synthetic mandate / benchmark id vocabulary. Decoupled from the stress + audit surface; may ship in parallel after v1.23 / v1.24. | Roadmap candidate. |
