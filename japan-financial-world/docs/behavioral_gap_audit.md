...
| **v1.9.8** | **Performance Boundary / Sparse Traversal Discipline.** Discipline milestone — no new behaviour, no new mechanism. Adds `docs/performance_boundary.md` and `tests/test_living_reference_world_performance_boundary.py`. Pins the per-run total record budget (for the default 4-period sweep), exact mechanism counts (`P × F` pressure, `P × I × F` valuation, `P × B × F` credit review), and forbids `ORDER_SUBMITTED` / `PRICE_UPDATED` / `CONTRACT_*` / `OWNERSHIP_TRANSFERRED` records on the default sweep. Documents future sparse-gating principles for production scale. No native rewrite in scope. | **Shipped (§68).** |
| **v1.9.last** | **Public prototype freeze.** Docs-only milestone. Adds `docs/v1_9_public_prototype_summary.md`, refreshes README + `RELEASE_CHECKLIST.md`, flips `public_prototype_plan.md` from plan-only to shipped, adds §69 to `world_model.md`. No runtime change. | **Shipped (§69).** |
