...
| v1.17.2     | Regime Comparison Report — adds `NamedRegimePanel` + `RegimeComparisonPanel` immutable dataclasses, three deterministic helpers (`build_named_regime_panel(...)`, `build_regime_comparison_panel(...)`, `render_regime_comparison_markdown(...)`), the closed-set `COMPARISON_AXIS_LABELS` frozenset (8 axes), and a new `examples/reference_world/regime_comparison_report.py` driver that runs each v1.11.2 regime preset on a fresh kernel and walks the read-only book interface to extract closed-loop label histograms; +35 tests | Shipped                 |
| **v1.17.3** | **Event Annotation + Causal Timeline Inspector** — adds two deterministic helpers to `world/display_timeline.py` (`build_event_annotations_from_closed_loop_data(...)` and `build_causal_timeline_annotations_from_closed_loop_data(...)`) that read **anonymous record-like inputs** (duck-typed via `getattr`, no source-of-truth-book imports) and emit `EventAnnotationRecord` + `CausalTimelineAnnotation` tuples using a closed-set rule table — Rule 1 `MarketEnvironmentStateRecord.overall_market_access_label = selective_or_constrained` → `market_environment_change` event with credit / funding / liquidity / volatility / refinancing-window subfields recorded in `metadata` AND included in the human-readable `annotation_label` (this is the differentiator that surfaces a visible difference between two regimes whose `overall_market_access_label` collide, e.g. `constrained` vs `tightening`); Rule 2 `IndicativeMarketPressureRecord.market_access_label ∈ {constrained, closed}` → `market_pressure_change` event (severity `high` for `closed`, `medium` for `constrained`); Rule 3 `CorporateFinancingPathRecord.constraint_label = market_access_constraint` → `financing_constraint` event; Rule 4 `CorporateFinancingPathRecord.coherence_label = conflicting_evidence` → `causal_checkpoint` event; Rule 5 `ActorAttentionStateRecord.focus_labels` contains any of `{risk, financing, market_access, information_gap, dilution}` → `attention_shift` event; the causal helper renders three plain-id arrows that already exist in the kernel (env → pressure via `source_market_environment_state_ids`; pressure → financing path via `indicative_market_pressure_ids`; prior pressure / path → next-period attention via `source_indicative_market_pressure_ids` / `source_corporate_financing_path_ids`); `NamedRegimePanel` gains two new tuple fields (`event_annotations`, `causal_annotations`); the markdown renderer adds three new comparison rows (Event annotations by type / Top events date · type · source / Causal arrows by kind) and a per-regime "Events & causal trace" block under the table that lists up to 6 top events + 6 causal arrows with concrete record ids and dates; the regime-comparison driver wires the helpers into `extract_regime_run_snapshot` so every regime panel carries the annotations; +31 tests across `tests/test_display_timeline.py` (+26: per-rule firing including the env subfield differentiator, helper determinism, no-fire on negative cases, panel acceptance + rejection of non-annotation entries, markdown event/causal section rendering, no-forbidden-display-name in event-enriched markdown) and `tests/test_regime_comparison_report.py` (+5: snapshot carries event/causal annotations, env-change metadata distinguishes constrained vs tightening on subfield labels, default-args markdown renders event section, two collision regimes show distinct trace blocks, no-forbidden-trade-keys in default-args markdown) | **Shipped**             |
...
- integration-test `living_world_digest`:
  **`f93bdf3f4203c20d4a58e956160b0bb1004dcdecf0648a92cc961401b705897c`**
  ...
  at v1.17.2);
- pytest count: **4165 / 4165** passing — up from 4033 / 4033
  at v1.16.last. The v1.17.1 milestone added +66 tests in
  `tests/test_display_timeline.py`; v1.17.2 added +35 more
  (+18 in `tests/test_display_timeline.py` for the regime-
  panel data model and +17 in
  `tests/test_regime_comparison_report.py` for the driver);
  v1.17.3 adds +31 more (+26 in `tests/test_display_timeline.py`
  for the event + causal helpers and the panel / markdown
  extensions, and +5 in `tests/test_regime_comparison_report.py`
  for the driver-side annotation extraction and the
  collision-regime markdown differentiator).
